...
public class EnvioImp {
    ...
    public static HashMap<String, Object> obtenerRecibidosSucursal() {
        HashMap<String, Object> respuesta = new LinkedHashMap<>();
        String URL = Constantes.URL_WS + "envio/obtener-recibidos-sucursal";
        ...
        if (respuestaAPI.getCodigo() == HttpURLConnection.HTTP_OK) {
            ...
            respuesta.put(
                    ...
                    Utilidades.obtenerMensajeErrorHTTP(
                            respuestaAPI.getCodigo(),
                            "Lo sentimos, no fue posible obtener los envíos recibidos en sucursal."
                    )
            );
        }
    ...
    public static HashMap<String, Object> buscarPorGuia(String numeroGuia) {
        ...
        if (respuestaAPI.getCodigo() == HttpURLConnection.HTTP_OK) {
            Gson gson = new Gson();
            Envio envio = gson.fromJson(respuestaAPI.getContenido(), Envio.class);

            respuesta.put(Constantes.KEY_ERROR, false);
            respuesta.put(Constantes.KEY_OBJETO, envio);
        } else {
            respuesta.put(Constantes.KEY_ERROR, true);
            respuesta.put(
                Constantes.KEY_MENSAJE,
    ...
    public static Respuesta actualizarEstatus(Envio envio) {
        ...
        if (respuestaAPI.getCodigo() == HttpURLConnection.HTTP_OK) {
            ...
            respuesta.setMensaje(
                    Utilidades.obtenerMensajeErrorHTTP(
                            respuestaAPI.getCodigo(),
                            "No fue posible actualizar el estatus del envío."
                    )
            );
        }
